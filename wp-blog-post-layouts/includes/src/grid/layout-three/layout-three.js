        ...
        const content = posts.map( ( post, index ) => {
            ...
            return (
                <article id={`post-${post.id}`} className={ `cv-post post-format--${getformat}` }>
                    ...
                    <div className="cv-post-cat-tag-wrap cv-clearfix">
                        {
                            ( () => { 
                                if( hascategories ) {
                        ...
                        }
                        {
                            ( () => { 
                                if( hastags ) {
                                    return <div className="cv-post-tags-wrap cv-post-meta-item">{ gettagids.map( ( gettagid ) => {
                                        return <Fragment><a href={gettags[gettagid].link} onClick={ ( e ) => e.preventDefault() }>{ gettags[gettagid].name }</a><span>{ escapeHTML( __( ',', 'wp-blog-post-layouts' ) ) } </span></Fragment>
                                    } ) }</div>
                                }
                            }) ()
                        }
                    ...
                    <div className={ `cv-post-meta${getmetaIcon}` }>
                        { ( (typeof post.author !== 'undefined' ) && authorOption ) &&
                            <span className="cv-author-meta cv-post-meta-item">
                                <span className="cv-author-thumb">
                                    <img src={ author_image } alt={ author_name }/>
                                </span>
                                <span className="cv-post-author-name">{ escapeHTML( __( 'By ', 'wp-blog-post-layouts' ) )  }<a href={author_url} onClick={ ( e ) => e.preventDefault() }>{ author_name }</a></span>
                            </span>
                        }
